...
exports.forgotPassword = AsyncErrorHandler(async (req,res,next)=>{
    ...
    const resetPassUrl = `${req.protocol}://${req.get("host")}/api/v1/artist/password/reset/${resetToken}`;
...
exports.resetPassword = AsyncErrorHandler(async(req,res,next)=>{
    ...
    const artist = await Artist.findOne({resetPasswordToken, resetPasswordExpire:{$gt:Date.now()}})
    if(!artist){
