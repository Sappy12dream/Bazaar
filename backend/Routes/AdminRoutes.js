...
router
  .route("/artist/:id")
  .get(isAuth, authRoles("admin"), getArtistDetail)
  .put(isAuth, authRoles("admin"), updateArtistRole)
  .delete(isAuth, authRoles("admin"), DeleteArtist);
router.route("/users").get(isAuth, authRoles("admin"), getAllUsers);
router
  .route("/user/:id")
  .get(isAuth, authRoles("admin"), getUserDetail)
